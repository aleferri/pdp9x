...
ZERO:   .dd    0
SIZE1:  .dd    8191
BIASW:  .dd    122881          ; 131072 - (SIZE+1)
FLAGB:  .dd    16383           ; flags base - BIAS, mod 2^17
...
; flags[0..SIZE] = 1, index lives in IX
        LIX     ZERO
        LAC     ONE
FILL:   DAC     (FLAGP), X
        TADX    ONE
        SXD     SIZE1           ; skip while IX != 8191
        JMP     FEND
        JMP     FILL
FEND:
...
SLOOP:  LIX     IVAR
        ...
        TAD     IVAR            ; k = i + prime
        TAS     BIASW           ; k + BIAS, L = 1 iff k > SIZE
        SKNL
        JMP     MDONE
        DAC     KVAR
        LIX     KVAR
        CLA
MARK:   DAC     (FLAGB), X
        TASX    PRIME           ; L = 1 exactly when k crosses SIZE
        SKL
        JMP     MARK
MDONE:  LAC     COUNT
        TAD     ONE
